...
const CryptoTable: React.FC = () => {
  ...
  const handleAddFav = (currency: DataTableCellClickEvent<Currency[]>) => {
    const target = currency.originalEvent.target.id;
  ...
  const imageBodyTemplate = (currency: Currency) => {
    return (
      <div className="flex items-center justify-between min-w-8 gap-2 relative pt-2 pb-2">
    ...
    <div className="flex flex-col h-fit pt-4 pb-4">
      {currencies.length > 0 ? (
        <DataTable
          ...
          onSelectionChange={(e) => handleAddFav(e)}
